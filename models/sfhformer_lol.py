# -----------------------------------------------------------------------------------
# 🚀 2025 冲刺版：Parallel (CNN + Mamba) + SKFF(r=2) + GatedRefine(Fixed) + NoHIN
# -----------------------------------------------------------------------------------
import math
import torch
...
# 🌟 新增导入 Mamba 模块
try:
    from .vmamba import SS2D  # 被 train_for_lolv2s.py 调用时走这里
except ImportError:
    from vmamba import SS2D   # 你自己单独运行 sfhformer_lol.py 时走这里

# ===================================================================================
# 1. SwinIR 基础组件 (保留定义，以防其他部分有隐式调用)
# ===================================================================================
class Mlp(nn.Module):
...
# ⭐⭐⭐ 核心修改点：重写 ParallelStage，用 Mamba 替换 RSTB ⭐⭐⭐
class ParallelStage(nn.Module):
    def __init__(self, depth, in_channels, mixer_kernel_size=[1,3,5,7], local_size=8):
        ...
        # 2. 引入 Mamba (SS2D) 分支
        self.mamba_branch = SS2D(
            d_model=in_channels, 
            d_state=16,        # Mamba 内部状态维度
            ssm_ratio=2.0,     # 通道扩展比例
            d_conv=3,          # 内部深度卷积的核大小
            channel_first=True, # 重要：让它接受和 CNN 一样的 (B, C, H, W) 图像格式
            forward_type="v3"  # 🌟🌟🌟 核心修复：指定使用编译成功的 oflex 版本算子
        )
    ...
    def forward(self, x):
        ...
        # 3. 拼接与融合
        x_cat = torch.cat([x_cnn, x_mamba], dim=1)
...
if __name__ == "__main__":
    print("🔍 Testing SOTA 2025 Model: Parallel (CNN+Mamba) + SKFF(r=2) + GatedRefine(Fixed)")
    model = sfhformer_t()
    x = torch.randn(2, 3, 64, 64)
    
    # 尝试移动到 GPU 进行测试 (Mamba通常需要CUDA环境)
